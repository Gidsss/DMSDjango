...
def view_post(request, pk):
    try:
        ...
        if post.file_data:
            ...
            # Use the Decoder class to decode the steganographic text to binary bitstream
            decoder = Decoder(model, post.file_data)
            ...
            # Return the decoded data for preview (or as part of response)
            response_data = {
                'status': 'success',
                'data': {
                    'file_name': post.title,
                    'binary_data': decoded_bitstream,  # You may also return the binary as text or for preview
